...
export default function HomeScreen() {
    ...
    const {sendMessage, isReconnecting, setIsReconnecting, connectedDevice, lastDevice, hasBonded, hasTried, connectDevice, disconnectDevice, unsubscribeRx} = DEV_MODE ? mockBLE : realBLE;
    ...
    // DEV: on first render in dev mode, seed storage and jump straight to device 0
    useEffect(() => {
        if (!DEV_MODE) return;
        const seed = async () => {
    ...
    useEffect(() => {
        if (DEV_MODE) return; // handled above
    ...
    useFocusEffect(
        useCallback(() => {
            if (DEV_MODE) return; // dev mode skips this screen entirely
    ...
    useEffect(() => {
        ...
        const init = async () => {
            if (hasBonded === 0) {
                ...
                setDevices(prev => prev.filter(itemm => itemm.deviceId !== lastDevice))
            }
        }
        init()
    }, [hasBonded]);

    // DEV: render nothing while redirecting (avoids a flash of the home screen)
    ...
    return (
        <View style={styles.container}>
            <ImageBackground
                ...
                //@ts-ignore
                imageStyle={{ filter: 'brightness(0.2)' }}
                resizeMode="cover"
            >
                <View style={localStyles.wrapper}>
                    ...
                    <View style={{ gap: 16 }}>
                                    ...
                                    <ContextMenu.Items>
                                        <Button
                                            ...
                                            onPress={async () => {
                                                ...
                                                setDevices(prev => prev.filter(itemm => itemm.id !== parseInt(String(item.id))))
                                            }}
                                        />
                                    </ContextMenu.Items>
                                    <ContextMenu.Trigger>
                                                    ...
                                                    <Host>
                                                        <Button
                                                            ...
                                                            modifiers={[
                                                                padding({ all: 4 }),
                                                                glassEffect({
                                                                    glass: { tint: 'rgba(255,255,255,0.2)', variant: 'regular', interactive: true },
                                                                    shape: 'capsule'
                                                                }),
                                                            ]}
                                                        >
                                                            <Text style={[localStyles.text, {
                                                                paddingBottom: 12, paddingVertical: 4, paddingHorizontal: 8,
                                                                paddingRight: 14, fontSize: 18, fontWeight: '600'
                                                                ...
                                                                if (!connectedDevice || item.deviceId !== connectedDevice?.id) {
                                                                    ...
                                                                    if (!connectedDevice) {
                                                                        try {
                                                                            ...
                                                                            if (state !== 'PoweredOn') {
                                                                                setFailed(1); setIsReconnecting(false); navigating.current = false; return;
                                                                            }
                                                                            const device = await manager.connectToDevice(item.deviceId, { autoConnect: false, timeout: 10000 });
                                                                            await device.discoverAllServicesAndCharacteristics();
                                                                            unsubscribeRx();
                                                                            const res = await connectDevice(device);
                                                                            goDevice.current = item.id;
                                                                        } catch (error: any) {
                                                                            setFailed(2); setIsReconnecting(false); navigating.current = false;
                                                                        }
                                                                    }
                                                                }
                                                                if (connectedDevice) {
                                                        ...
                                                        >
                                                            <View style={[{
                                                                ...
                                                                <View style={localStyles.deviceItemDetails}>
                                                                    <Host matchContents>
                                                                        <Pod style={{ alignSelf: "center" }} height={'90%'}/>
                                                                    </Host>
                                                                    <View style={localStyles.deviceItemStuff}>
                                                                        <View>
                                                                            <Text style={[localStyles.text, localStyles.headline]}>{item.name}</Text>
                                                                            <Text style={[localStyles.text, localStyles.subheadline, localStyles.greyed]}>Acoustic Pod</Text>
                                                                        </View>
                        ...
                        <GlassView style={[localStyles.glassBox, localStyles.deviceItemOuterOuter]}
                                   tintColor={'rgba(50,50,50,.7)'} glassEffectStyle="clear">
                            <GlassView style={[localStyles.glassBoxBox, { outlineWidth: 1, outlineColor: 'rgba(255,255,255,0.5)' }]}
                                       glassEffectStyle="clear" tintColor={'rgba(50,50,50,.5)'}>
                                <Pressable onPress={() => router.push('/pairing')} style={{ width: '100%', height: '100%' }}>
                                    <Host style={{ width: '100%', height: '100%' }} hitSlop={0}>
                                        <Button
                                            label="Add Device" systemImage="plus" variant="plain"
                                            onPress={() => router.push('/pairing')}
                                            modifiers={[
                                                labelStyle('iconOnly'), font({ size: 48 }), foregroundStyle('white'),
                                                frame({ maxWidth: 10000, maxHeight: 10000 }),
                                                glassEffect({ glass: { variant: 'regular', interactive: true }, shape: 'rectangle' }),
                                            ]}
                                        />
                                    </Host>
                                </Pressable>
                ...
                {failed > 0 ? (
                    <View style={{
                        ...
                        paddingVertical: 84, paddingHorizontal: 24, justifyContent: 'flex-end', alignItems: 'center'
                    }}>
                        <GlassView style={[localStyles.warningContainer]} glassEffectStyle="clear" tintColor="rgba(50,50,50,.5)">
                            <IconSymbol size={40} color="#EAAC47" name="exclamationmark.triangle.fill"/>
                            <Text style={[localStyles.text, { flexShrink: 1, flexGrow: 1, width: 0 }]}>
...
const localStyles = StyleSheet.create({
    wrapper: { width: "100%", height: "100%", paddingHorizontal: 16, paddingTop: 72, gap: 10 },
    deviceItemStuff: { height: "100%", justifyContent: "space-between", flexDirection: 'column' },
    footnote: { fontSize: 13, fontWeight: "bold" },
    hertzTag: { paddingHorizontal: 8, paddingLeft: 6, paddingTop: 3, paddingBottom: 4, flexDirection: "row", alignItems: "center", gap: 4, borderRadius: 12 },
    titleBox: { width: "100%", justifyContent: "space-between", flexDirection: "row", alignItems: "center" },
    greyed: { color: "#C5C5C5" },
    subheadline: { fontSize: 15, fontWeight: "regular" },
    deviceItemDetails: { flexDirection: "row", height: "100%", gap: 24 },
    headline: { fontSize: 17, fontWeight: "bold" },
    largeTitle: { fontSize: 34, fontWeight: "bold" },
    titleContainer: { gap: 10 },
    deviceItemInner: { alignItems: "center", justifyContent: "center", width: "100%", height: "100%", borderRadius: 16 },
    deviceItem: { display: "flex", flexDirection: "row", alignItems: "center", justifyContent: "space-between", paddingHorizontal: 16, paddingVertical: 12 },
    deviceItemOuterOuter: { padding: 12 },
    glassBox: { width: '100%', height: 106, borderRadius: 24, overflow: "hidden", alignItems: "center", justifyContent: "center" },
    glassBoxBox: { width: '100%', height: '100%', borderRadius: 16, overflow: "hidden", alignItems: "center", justifyContent: "center" },
    warningContainer: { paddingHorizontal: 24, paddingVertical: 16, marginBottom: 32, flexDirection: 'row', alignItems: 'center', gap: 24, borderRadius: 24, width: '100%' },
    text: { color: "#fff" },
});